...
| TASK-121 | **high** | ⏳ **Parent, in progress** — put Dependabot on a working footing. 422 alerts and 38 stale PRs because `open-pull-requests-limit` does not apply to security updates. Sub-tasks: **TASK-122** pin + dedupe (388 of 392 specifiers are `^`; only 3 deps conflict), **TASK-123** rules (ignore majors — *scoped*, or security fixes get suppressed too), **TASK-124** automate merging + regenerate the backlog rather than grinding 38 PRs, **TASK-125** transitive strategy (parent-bump first, `overrides` only as fallback) |
