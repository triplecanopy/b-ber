...
- [x] **Audit existing Dependabot PRs / alerts** on GitHub; close or dismiss
      any stale ones before making changes.
- [x] **Decide on approach** (Options A–D above, or hybrid).
- [x] **Implement chosen option**: Chose Option B (reconfigure Dependabot with
      correct target branch, grouping, and version updates enabled). Rewrote
      `.github/dependabot.yml` with correct branch targeting, dependency
      grouping, and `github-actions` ecosystem entry.
- [x] **Document the chosen approach** in root `AGENTS.md` under a new
...
**Implementation (2026-06-19):** Chose Option B (reconfigure, not remove).
Rewrote `.github/dependabot.yml` with the following changes:

- **Target branch:** Removed `target-branch: 'develop'` (branch does not exist).
  Dependabot will now target the repo default branch (`main`). PRs can be
  rebased to `feat/upgrades` as needed.
- **Version updates enabled:** Changed `open-pull-requests-limit` from `0` to
  `10` for the `npm` ecosystem. Set limit `5` for `github-actions`.
- **Dependency grouping (npm):** Added named groups — `babel` (`@babel/*`),
  `types` (`@types/*`), `testing` (`jest*`, `@jest/*`), `linting` (`biome`,
  `@biomejs/*`, `eslint*`), `build-tools` (`webpack*`, `vite*`, `rollup*`,
  `tsdown*`), `react` (`react`, `react-*`, `react-dom`, `react-redux`,
  `@reduxjs/*`), and a catch-all `minor-patch` group for all remaining
  minor/patch bumps.
- **GitHub Actions ecosystem:** Added a second `updates` entry for
  `github-actions` with weekly schedule, same `security` label and `msimmer`
  assignee, and a single catch-all group.
- **Preserved:** weekly schedule, `security` label, `msimmer` assignee.
