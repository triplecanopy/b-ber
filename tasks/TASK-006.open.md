...
- [ ] CSS Modules migration (TASK-017 scope) — deferred; see Notes
...
- **CSS Modules (TASK-017)**: The CSS Modules migration is deferred. The current SCSS
  structure (global partials in `src/styles/`) works correctly with Vite without
  any changes. CSS Modules would require significant component refactoring. TASK-017
  remains open.
- **SCSS deprecation warnings**: Dart Sass 3.x deprecation warnings appear for
  `@import` and global built-in functions in `src/styles/_mixins.scss`. These are
  pre-existing and do not affect the build output. Will be addressed separately.
