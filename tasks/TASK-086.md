# TASK-086: Reset view.loaded reliably and remove the dead unload path

**Epic:** React 19 (reader-react)
**GitHub Issue:** #628 — https://github.com/triplecanopy/b-ber/issues/628
**Phase:** Bug Fixes
**Created:** 2026-06-06
...
- TASK-085 had to dispatch `unload()` on the resize path specifically to re-arm
...
- restore a non-racy `loading → loaded` edge so chapter change, resize, and
  fullscreen all share one mechanism (superseding the TASK-085 workaround).
...
- [ ] Fold the TASK-085 resize workaround into the shared mechanism if applicable
- [ ] Audit `media.js` / backward-nav consumers of `view.loaded`
- [ ] Verify chapter nav (fwd/back), resize, and fullscreen across browsers
- [ ] `npm test`; commit; PR with `Closes #628`

## Notes

- Related: TASK-085 (resize spinner workaround), TASK-081/024 (consume the
