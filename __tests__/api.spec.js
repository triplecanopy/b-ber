...
function task_1(throwTopLevelError = false, throwNestedError = false) {
  return new Promise(resolve => {
    ...
    const promises = dirs.map(
      a => fs[methodName](path.join(cwd, a)).catch(logger_handleFatalError) // eslint-disable-line no-undef
    )
...
function task_2(response) {
  return new Promise(resolve => {
    ...
    const dirs = ['./']
    const promises = dirs.map(
      a => fs.mkdirp(path.join(cwd, a)).catch(logger_handleFatalError) // eslint-disable-line no-undef
    )
...
describe('b-ber api', () => {
  ...
  test('handles top-level errors', done => {
    ...
    cp.stdout.on('data', data =>
      expect(String(data)).toMatch(/Error: err message/)
    )
  ...
  test('handles nested errors', done => {
    ...
    cp.stdout.on('data', data =>
      expect(String(data)).toMatch(/Error: EEXIST: file already exists/)
    )
