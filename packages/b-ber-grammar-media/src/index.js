...
import { attributesString, attributesObject, htmlId, toAlias } from '@canopycanopycanopy/b-ber-grammar-attributes'
import {
    isHostedRemotely,
...
function prepare({ token, marker, context, instance, fileName, lineNumber }) {
    ...
    if (!source) log.error(`Directive [${type}] requires a [source] attribute at [${fileName}:${lineNumber}]`)
...
const render = ({ instance, context }) => (tokens, index) => {
    ...
    const type = marker[1]
    const args = prepare({ token, marker, context, instance, fileName, lineNumber })
