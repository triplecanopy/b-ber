...
import {modelFromString, modelFromObject} from './utils'
...
const flattenNestedEntries = (arr, result = []) => {
    arr.forEach(a => {
        if (isPlainObject(a)) { // in an entry
            ...
            throw new Error(`[state#flattenNestedEntries] requires an array of Objects, [${typeof _}] provided`) // eslint-disable-line max-len
        }
    })
