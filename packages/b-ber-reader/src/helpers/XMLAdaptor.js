...
class XMLAdaptor {
    ...
    static udpateSpineItemURLs(rootNode, opsURL) {
        return new Promise(resolve => {
            ...
            spine.map(a => a.absoluteURL = Url.resolveRelativeURL(opsURL, a.href)) // eslint-disable-line no-param-reassign
            resolve({...rootNode, spine})
        })
    }
    static udpateGuideItemURLs(rootNode, opsURL) {
        return new Promise(resolve => {
            ...
            guide.map(a => a.absoluteURL = Url.resolveRelativeURL(opsURL, a.href)) // eslint-disable-line no-param-reassign
            resolve({...rootNode, guide})
        })
    }
    ...
    static createBookMetadata(rootNode) {
        ...
        const _metadata = __metadata.elements.filter(a => /^dc:/.test(a.name)).map(b => ({[b.name.slice(3)]: b.elements[0].text}))
                ...
                sheets.forEach(sheet => {
                    ...
                    csstree.walk(ast, {
                        enter: (node, item, list) => {
                            ...
                            const scopedClassName = List.createItem({name: hashedClassName, type: 'ClassSelector'})
                            const whiteSpace = List.createItem({type: 'WhiteSpace', value: ' '})
