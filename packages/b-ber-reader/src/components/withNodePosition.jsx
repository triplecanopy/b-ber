...
function withNodePosition(WrappedComponent, { useParentDimensions }) {
    return class extends React.Component {
        ...
        componentDidMount() {
            this.calculateNodePositionAfterResize = debounce(this.calculateNodePosition, 0, {
                leading: false,
                trailing: true,
            }).bind(this)
        ...
        elementEdgeIsInAllowableRange(edgePositionVariance) {
            const result =
                (edgePositionVariance >= ELEMENT_EDGE_VERSO_MIN && edgePositionVariance <= ELEMENT_EDGE_VERSO_MAX) ||
                (edgePositionVariance >= ELEMENT_EDGE_RECTO_MIN && edgePositionVariance <= ELEMENT_EDGE_RECTO_MAX)
        ...
        calculateNodePosition() {
            ...
            const { marginLeft, paddingLeft: elementPaddingLeft } = window.getComputedStyle(elem)

            // Get the left edge of the element, taking into account padding and margins
            const elementEdgeLeft = elem.offsetLeft - parseFloat(marginLeft) - parseFloat(elementPaddingLeft)
            ...
            // recto position
            const elementEdgeLeftInRecto = elementEdgeLeft - columnGap - columnWidth - paddingLeft
            ...
            const edgePositionVariance = Number((edgePosition % 1).toFixed(2).substring(2))
        ...
        render() {
            return <WrappedComponent elemRef={this.elemRef} {...this.state} {...this.props} />
        }
    }
}
