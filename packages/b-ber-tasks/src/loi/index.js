...
const createLOILeader = () => {
    ...
    return fs.writeFile(path.join(state.dist, 'OPS', 'text', `${fileName}`), markup, 'utf8')
        .then(() => {
...
const createLOIAsSeparateHTMLFiles = () => {
    const promises = state.figures.map(data => {
        ...
        return fs.writeFile(path.join(state.dist, 'OPS', 'text', data.page), markup, 'utf8')
            .then(() => {
                const fileData = new SpineItem({fileName: data.page, in_toc: false, ref: data.ref, pageOrder: data.pageOrder})
