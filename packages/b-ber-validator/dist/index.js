...
function text() {
  return F.not(fence()).optrep().drop();
}
...
function fence() {
  return lf().then(sep()).then(sep()).then(sep()).then(space()).drop();
}

function ident() {
  return F.satisfy(c => 'a' <= c && c <= 'z' || 'A' <= c && c <= 'Z' || '-' === c || '_' === c).rep().map(cs => cs.join(''));
}

// TODO support unicode - should also have `sentence()`
function word() {
  return F.satisfy(c => 'a' <= c && c <= 'z' || 'A' <= c && c <= 'Z' || '-' === c || '_' === c || c === ' ').rep().map(cs => cs.join(''));
}
...
function enter() {
  return fence().then(open()).then(sep().drop()).then(ident());
}

// TODO support unicode, ', ", etc. Also requires 'wrapping' sentences in
// matching quotes, and omitting the boundaries
function attr() {
  return C.letters().then(sep().drop()).then(C.charIn(["'", '"']).opt().drop()).then(F.try(C.letters()).or(word())).then(C.charIn(["'", '"']).opt().drop());
}

function attrs() {
  return space().drop().then(attr()).rep();
}

function exit() {
  return fence().then(close()).then(sep());
}
...
function block() {
  return text().then(enter())

  // Return id from previous `ident()` call
  .map(({ value }) => value[1]).then(attrs())
