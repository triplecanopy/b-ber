...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `src/index.js`        | Barrel export for all template classes                                                                                                    |
| `src/Ncx/`            | EPUB 2 NCX navigation document (head, docTitle, navMap, navPoints)                                                                        |
| `src/Opf/Pkg.js`      | OPF `<package>` wrapper element                                                                                                           |
| `src/Opf/Metadata.js` | OPF `<metadata>` block                                                                                                                    |
| `src/Opf/Manifest.js` | OPF `<manifest>` item list                                                                                                                |
| `src/Opf/Spine.js`    | OPF `<spine>` reading order                                                                                                               |
| `src/Opf/Guide.js`    | OPF `<guide>` landmarks                                                                                                                   |
...
| `src/figures/`        | Figure markup dispatcher; delegates to `epub.js`, `mobi.js`, `reader.js`, `web.js` by media type and build target                         |
| `__tests__/`          | Jest snapshot tests for Toc, Xhtml, Xml, Project, and all figure variants                                                                 |
| `Ncx/`, `Opf/`, etc.  | Built output directories (generated by `npm run build`; do not edit by hand)                                                              |
...
- All template logic lives in `src/`; compiled output directories (`Ncx/`, `Opf/`, etc.) are generated and must not be edited by hand
