...
export function opfURL(url: string): string {
  let url_ = url
...
export function parseOPF(xml: { data: string }): Promise<RootNode> {
...
export function createSpineItems(rootNode: RootNode): Promise<RootNode> {
  ...
  return new Promise((resolve) => {
    ...
    if (__ncx) {
      ...
      navMap.elements.forEach((navPoint: AnyNode) => {
        parseNavPoints(spine, __manifest, navPoint)
      })
    }
...
export function createGuideItems(rootNode: RootNode): Promise<RootNode> {
...
export function udpateSpineItemURLs(
  rootNode: RootNode,
  opsURL: string
): Promise<RootNode> {
  return new Promise((resolve) => {
    ...
    spine.map(
      (a: SpineItem) => (a.absoluteURL = Url.resolveRelativeURL(opsURL, a.href))
    )
    resolve({ ...rootNode, spine })
  })
}

export function udpateGuideItemURLs(
  rootNode: RootNode,
  opsURL: string
): Promise<RootNode> {
  return new Promise((resolve) => {
    ...
    guide.map(
      (a: GuideItem) => (a.absoluteURL = Url.resolveRelativeURL(opsURL, a.href))
    )
    resolve({ ...rootNode, guide })
  })
}
...
export function parseNavPoints(
  ...
  navPoint.elements.forEach((child: AnyNode) => {
    parseNavPoints(spine, manifest, child, depth_, spineItem)
  })
}

export function createBookMetadata(rootNode: RootNode): Promise<RootNode> {
...
export async function parseSpineItemResponse(
  response: AnyNode
  ...
  const styles = links.reduce((acc: { url: string; base: string }[], curr) => {
  ...
  const scopedCSS = createScopedCSS(sheets, hashedClassName, opsURL)
