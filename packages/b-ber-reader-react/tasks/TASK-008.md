...
the `spineItemURL` key from TASK-006) and showed correct content.
...
- [x] Verify whether Bug A is resolved by the TASK-006 spineItemURL fix
...
- Bug A root cause was `viewActions.unload()` in `freeze()` causing OLD Ultimate to
  fire prematurely. The TASK-006 `spineItemURL` key fix ensured correct content eventually
