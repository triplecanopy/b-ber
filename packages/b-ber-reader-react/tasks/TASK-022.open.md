# TASK-022: Fix stale full-bleed spread column position (verso/recto)
...
- Depends on TASK-023 (reliable `Ultimate` settle signal) — without it the
...
- Files changed: `src/components/Spread.jsx`, `src/components/SpreadFigure.jsx`,
  `src/lib/spread-context.js`, plus `Viewport.getPageWidth` (see TASK-025).
