...
`b-ber-cli` is the main entry point for all CLI commands. Commands are issued to `bber` via [`yargs`](https://www.npmjs.com/package/yargs), which then dispatches the appropriate handler in `b-ber-cli/cmd`.
...
1. Passes the `build` command to `yargs` in `b-ber-cli`
2. `b-ber-cli/index.js` looks for a task registered with the name `build` in `b-ber-cli/cmd/build.js`
3. `bber` finds and invokes the `build` handler in `b-ber-cli/cmd/build.js`
4. The handler calls the appropriate method exported by `bber-output/build.js` while passing along any additional arguments

## Stand-Alone Commands

Many `bber` commands can be run as stand-along commands.
...
Chaining tasks is done using `Promises`. The `build` command calls a sequence of `Promise`s serially to build an ebook. Every `bber` command returns a `Promise` object.
...
<tr><td><a href="https://github.com/triplecanopy/b-ber-creator/tree/master/src/b-ber-shapes"><code>b-ber-shapes</code></a></td><td>Simple objects and arrays referenced by the application</td></tr>
