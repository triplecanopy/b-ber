...
describe('module:md:MarkdownRenderer', () => {
    ...
    describe('#render', () => {
        ...
        it('Should update the global state with a Markdown file\'s frontmatter', () => {
            const callback = sinon.spy(state, 'add')
            markdownRenderer.render('test', '--- foo: bar\n# Test')
            sinon.assert.calledOnce(callback)
            state.pages.should.be.an('array')
            state.pages[0].filename.should.equal('test')
            state.pages[0].should.have.property('foo')
        })
    })
